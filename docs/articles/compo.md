...
- над [s3 хранилищем](https://github.com/s3fs-fuse/s3fs-fuse)
- удаленные(remote) файловые системы: `sshfs`, `FTP`
- [цифры числа пи](https://github.com/philipl/pifs)
- [HTTP](https://rejetto.com/hfs/) как файловая система
- в [качестве](https://ondoc.logand.com/d/5736/pdf) файловой системы использовать состояние какой-то программы или [внутреннего](https://docs.kernel.org/filesystems/proc.html) состояния системы, т.е. когда по факту никаких файлов нигде нет.
- и так далее
...
- [http as kv db](https://unstorage.unjs.io/guide/http-server)
...
Еще один, вдохновивший меня, пример. На этот раз одна конкретная cli тулза на zsh-е, в которой реализована одна идея, которая, как мне кажется, была бы полезна много где. Тулза называется [sc](https://github.com/annacrombie/sc). Она древняя, и на zsh, и не работает, и я не помню как ее нашел. Но как то сохранил и недавно переписал [форк](https://github.com/rprtr258/sc) на го, чтобы более менее работало (UPD: оно снова не работает, мне впадлу чинить реверснутое кривое апи этого ебаного сервиса). В описании не сказано ничего про композицию
```
A lightweight soundcloud client, conforming to unix philosophy.
```
но сказано про `unix philosophy`, суть которой в том, чтобы "писать программы, которые могут работать вместе". Как по мне это хорошая цель, но плохая идея в свете "текст это универсальный интерфейс". Из за этого 50%(цифра взята с потолка) скриптов это чистка и парсинг данных из одной утилиты для другой. Напоминает [helm темплейтинг](https://github.com/prometheus-community/helm-charts/blob/main/charts/prometheus-blackbox-exporter/templates/service.yaml), в котором нужно каждый отступ и каждый ескейп сделать правильно, чтобы дай бог ничего не развалилось. Вместо этого всего, более жизнеспособными альтернативами видится [использование](https://github.com/kellyjonbrazil/jc) [json](https://jsonnet.org/)-а или даже вообще [типизированных](https://www.nushell.sh/) [представлений](https://en.wikipedia.org/wiki/PowerShell).
...
Абстрактно, картинка [это](https://pkg.go.dev/image#Image)
```go
struct Image {
    Width, Height int
    At(x, y int) Color
...
![](https://cdn.7tv.app/emote/01H2654R68000651A0D191BFDW/4x.avif)
...
![](https://cdn.7tv.app/emote/01GERMH9M0000BQ5E4E4CKHN7S/4x.avif)

В этом в принципе вся идея. У тсодинга есть похожий [проект](https://tsoding.github.io/emoteJAM/).

Идея мне настолько понравилась, что я придумал, но не реализовал (потому что это фронтенд и потому что libwebp кривая хуйня) [проект/язык](https://github.com/rprtr258/twitch-emotes-modifier-plugin) с стековой композицией эмоутов. Идея в том, чтобы добавить всякие разные модификаторы, в том числе `zero-width` эмоуты, композицию нескольких эмоутов и делать длмнные сложные выражения в виде эмоутов:

```
Watching dup stackx Ceiling stackx Resting vahui Ceiling stackx stackx Resting WatchingR dup stackx stackx stacky stacky
```

![](/articles/static/img/twemod.webp)
...
![](/articles/static/img/twemod2.webp)

По сути получился стековый язык над смайликами. Композируемый так же, как и другие конкатенативные языки.

## Noita wand building

Есть такая игра [Noita](https://steamcommunity.com/app/881100). Очень кратко - суть в том, чтобы собирать спелы и собирать из них более мощные спелы.

- базовые спелы([`projectile`](https://noita.wiki.gg/wiki/Spells#Projectile)) - просто создают снаряд, который летит и наносит урон
- модификаторы([`modifier`](https://noita.wiki.gg/wiki/Category:Spells_of_type_Projectile_modifier)) - берут следующий спел и меняют его параметры
- мультикаст([`multicast`](https://noita.wiki.gg/wiki/Spells#Multicast)) - берут несколько следующих спелов и кастуют их одновременно
- особые([`other`](https://noita.wiki.gg/wiki/Spells#Other)) - копируют спелы, меняют порядок каста и тд

Более подробное [описание](https://noita.wiki.gg/wiki/Expert_Guide:_Draw) механики каста выходит за скоуп данной статьи. Суть в том, что базовые спелы комбинируются в разные другии спелы, создавая некий "стековый" язык со своими правилами.

![](/articles/static/img/compo-noita.png)
